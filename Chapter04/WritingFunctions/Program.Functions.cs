...
partial class Program
{
    ...
    static void RunFibImperative()
    {
        for (uint i = 1; i <= 30; i++)
        {
            WriteLine($"The {i} term of the Fibonacci sequence is {i:N0}.",
                arg0: CardinalToOrdinal(i),
                arg1: FibImperative(term: i));
        }
    }

    static int FibFunctonal(uint term) => term switch
    {
        0 => throw new ArgumentOutOfRangeException(),
        1 => 0,
        2 => 1,
        _ => FibFunctonal(term - 1) + FibFunctonal(term - 2)
    };
